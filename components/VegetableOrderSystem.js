...
import { Tabs, TabsContent, TabsList, TabsTrigger } from '../src/components/ui/tabs';
import { Textarea } from '../src/components/ui/textarea';
import { Leaf, ShoppingBasket, AlertCircle } from 'lucide-react';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '../src/components/ui/select';
import { Alert, AlertDescription } from '../src/components/ui/alert';
...
const VegetableOrderSystem = () => {
    ...
    // Filter vegetables based on search input
    const filteredVegetables = useMemo(() => currentOrder.items.filter(item => item.name.toLowerCase().includes(searchText.toLowerCase())), [currentOrder.items, searchText]);
    // Calculate total quantity in kg (gms converted)
    const totalOrderQuantity = useMemo(() => {
    ...
    // Handle quantity input change, restrict non-numeric chars except decimal point
    const handleQuantityChange = (id, value) => {
        const numericValue = value.replace(/[^0-9.]/g, '');
        setCurrentOrder(prev => ({
            ...prev,
    ...
    const handleSubmitOrder = async () => {
        ...
        try {
            ...
            setHotelName('');
            // Wait 1.5 seconds then close dialog and switch tab
            setTimeout(() => {
                setShowDialog(false);
    ...
    return (_jsxs("div", { className: "flex justify-center my-8", children: [_jsxs(Card, { className: "w-full max-w-5xl bg-green-50 border-green-200", children: [_jsx(CardHeader, { className: "bg-green-100 rounded-t-lg flex items-center", children: _jsxs(CardTitle, { className: "flex items-center", children: [_jsx(Leaf, { className: "mr-2 text-green-600" }), "Kalamkar Exotics Order System"] }) }), _jsx(CardContent, { children: _jsxs(Tabs, { value: activeTab, onValueChange: setActiveTab, className: "space-y-4", children: [_jsxs(TabsList, { className: "bg-green-100 mb-4", children: [_jsxs(TabsTrigger, { value: "new-order", className: "flex items-center", children: [_jsx(ShoppingBasket, { className: "mr-2" }), " New Order"] }), _jsx(TabsTrigger, { value: "history", children: "Order History" })] }), _jsx(TabsContent, { value: "new-order", className: "min-h-[600px] w-[900px] mx-auto", children: _jsxs("div", { className: "space-y-4 w-full", children: [_jsx(Input, { placeholder: "Search vegetables...", value: searchText, onChange: (e) => setSearchText(e.target.value), className: "w-full pl-10 border-green-300 focus:ring-green-500" }), _jsx(Input, { placeholder: "Enter Hotel Name", value: hotelName, onChange: (e) => setHotelName(e.target.value), className: "w-full border-green-300 focus:ring-green-500" }), orderError && (_jsxs(Alert, { variant: "destructive", children: [_jsx(AlertCircle, { className: "h-4 w-4" }), _jsx(AlertDescription, { children: orderError })] })), _jsx("div", { className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4", children: filteredVegetables.map((item) => (_jsxs("div", { className: "flex items-center space-x-2 p-3 border rounded-lg bg-white hover:bg-green-50 border-green-200 shadow-sm", children: [_jsx("div", { className: "flex-grow", children: _jsx("div", { className: "font-medium", children: item.name }) }), _jsx(Input, { type: "number", value: item.quantity, onChange: (e) => handleQuantityChange(item.id, e.target.value), className: "w-20 border-green-300 focus:ring-green-500", placeholder: "Qty" }), _jsxs(Select, { value: item.unit, onValueChange: (unit) => handleUnitChange(item.id, unit), children: [_jsx(SelectTrigger, { className: "w-15", children: _jsx(SelectValue, { placeholder: "Unit" }) }), _jsxs(SelectContent, { children: [_jsx(SelectItem, { value: "gms", children: "gm" }), _jsx(SelectItem, { value: "kg", children: "kg" })] })] })] }, item.id))) }), _jsxs("div", { className: "bg-green-100 p-3 rounded-lg flex justify-between items-center", children: [_jsx("span", { className: "font-medium", children: "Total Order Quantity:" }), _jsxs("span", { className: "text-lg font-bold text-green-700", children: [totalOrderQuantity, " kg"] })] }), _jsx(Textarea, { value: currentOrder.note, onChange: (e) => setCurrentOrder((prev) => ({ ...prev, note: e.target.value })), placeholder: "Add any special instructions here...", className: "w-full border-green-300 focus:ring-green-500" }), _jsx(Button, { onClick: handleSubmitOrder, className: "w-full bg-green-600 hover:bg-green-700", children: "Submit Order" })] }) }), _jsx(TabsContent, { value: "history", className: "min-h-[600px] w-[900px] mx-auto", children: _jsx("div", { className: "space-y-4 w-full", children: orders.length === 0 ? (_jsx("div", { className: "text-center text-gray-500 py-8 w-full", children: "No orders yet. Place your first order!" })) : (orders.map((order) => (_jsx(Card, { className: "p-4 mb-4 bg-white border-green-200 shadow-sm", children: _jsxs("div", { className: "space-y-4", children: [_jsxs("div", { className: "flex justify-between items-center", children: [_jsxs("span", { className: "font-medium flex items-center", children: [_jsx(ShoppingBasket, { className: "mr-2 text-green-600" }), new Date(order.date).toLocaleDateString()] }), _jsx("span", { className: `text-sm px-2 py-1 rounded-full ${order.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}`, children: order.status })] }), _jsxs("div", { className: "font-bold", children: ["Hotel: ", order.hotelName] }), _jsx("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-2", children: order.items.map((item, idx) => (_jsxs("div", { className: "flex justify-between items-center", children: [_jsxs("div", { className: "flex items-center", children: [_jsx("img", { src: item.image || '/placeholder.png', alt: item.name, className: "w-8 h-8 object-cover rounded-md mr-2" }), _jsx("span", { children: item.name })] }), _jsxs("span", { children: [item.quantity, " ", item.unit] })] }, item._id || idx))) }), _jsxs("div", { className: "flex justify-between mt-2 text-sm", children: [_jsx("span", { children: "Total Quantity:" }), _jsxs("span", { className: "font-bold text-green-700", children: [order.totalQuantity, " kg"] })] }), order.note && (_jsxs("div", { className: "text-sm text-gray-600 italic border-l-4 border-green-300 pl-2", children: ["Note: ", order.note] }))] }) }, order._id)))) }) })] }) })] }), _jsx(Dialog, { open: showDialog, onOpenChange: setShowDialog, children: _jsxs(DialogContent, { className: "max-w-sm", children: [_jsx(DialogHeader, { children: _jsx(DialogTitle, { children: "Order Status" }) }), _jsx("div", { className: "text-center text-green-800 py-4", children: dialogMessage })] }) })] }));
};
export default VegetableOrderSystem;
